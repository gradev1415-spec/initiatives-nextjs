...
export function matchContent(ini){
  ...
  ini.sg.forEach(function(g){
    ...
    found.forEach(function(l){matches.push({type:"skill",gap:g.s,impact:g.i,n:g.n,content:l});});
    if(found.length===0) matches.push({type:"content_gap",gap:g.s,impact:g.i,n:g.n,content:null});
  });
  ini.cg.forEach(function(g){
    if(g.c==="All certs")return;
    var found=LIBRARY.filter(function(l){return l.ct&&(l.ct.indexOf(g.c.split(" ")[0])!==-1||l.ct===g.c);});
    found.forEach(function(l){matches.push({type:"cert",gap:g.c,impact:g.i,n:g.n,content:l});});
  });
  return matches;
}
