...
/* ═══════════════════════════════════════════════════════════════
   ...
   PILLAR 2 — Capability  (do staff have the required skills at target levels?)
     Initiative level: derived from skill gap data (sg[]).
       ...
       Readiness = (total skill slots − unmet slots) / total skill slots
       Weighted by gap impact: Critical=2, High=1.5, Medium=1, Low=0.5
     Area level: derived from area skillReqs and staff fill.
       If area has staff gaps, proportional skills are uncovered.
       All skill levels treated equally (no difficulty penalty).

   PILLAR 3 — Compliance  (do staff hold required valid certificates?)
     Initiative level: from certs[] data → valid / total
       Weighted by cert gap impact when cert gap data available.
...
/* Impact/criticality weight for skill/cert gaps */
function iw(impact) {
  if (impact === "Critical" || impact === "Essential") return 2;
  if (impact === "High" || impact === "Important") return 1.5;
  if (impact === "Medium") return 1;
  return 0.5; /* Low / Nice to have */
}
...
/* Initiative-level skill readiness from skill gaps data.
   Logic: each sg entry = N people with an unmet skill at a given criticality.
   We compute: for each gap, what fraction of staff is affected (n / totalStaff),
   then weight by criticality. Aggregate into a weighted average.
   A gap affecting 100% of staff at Essential criticality is worst case.
   Formula: sum of (gap_fraction * weight) / sum of (1.0 * weight) per gap entry.
   Then readiness = 1 - weighted_gap_fraction. */
export function skillRd(ini) {
  ...
  /* For each skill gap: compute gap severity = (people affected / total staff)
     weighted by criticality. Then average across all gaps.
     A fully-gapped Essential skill = maximum damage to readiness. */
  var wSum = 0, wGapSum = 0;
  sg.forEach(function(g) {
    var w = iw(g.i);
    var gapFraction = Math.min(1, g.n / totalReq);
    wGapSum += gapFraction * w;
    wSum += w;
  });

  if (wSum === 0) return Math.round(100 * fillRate);
  var avgGapFraction = wGapSum / wSum;
...
/* Initiative-level cert readiness from certs[] data (valid/total).
   Also factors in cert gaps (cg[]) weighted by impact. */
export function certRd(ini) {
  ...
  /* If we have cert gaps (cg), weight by their impact */
  var cg = ini.cg || [];
  if (cg.length > 0) {
    var wGap = 0, wPossible = 0;
    ...
    var gapPenalty = totalCerts > 0 ? Math.min(0.3, (wGap / totalCerts) * 0.15) : 0;
...
/* Area skill readiness — derived from staffing fill and skill requirements.
   ...
   Skill criticality weights the contribution: Essential skills
   matter more than Nice-to-have ones.
...
export function areaSkillRd(area) {
  ...
  /* Each skill requirement contributes weighted by criticality.
     Coverage = staff fill rate (if you have the people, skills are met).
     Criticality weight: Essential=2, Important=1, Nice to have=0.5 */
  var wTotal = 0, wMet = 0;
  reqs.forEach(function(req) {
    var cr = typeof req === "string" ? "Important" : (req.cr || "Important");
    var w = cw(cr);
    wTotal += w;
    wMet += fillRate * w;
  });

  if (wTotal === 0) return Math.round(fillRate * 100);
  return Math.max(0, Math.min(100, Math.round((wMet / wTotal) * 100)));
}
...
/* Area cert readiness — derived from cert requirements and staff fill.
   ...
   Cert criticality determines how much each cert requirement matters.
   Coverage equals staff fill — if staff are in place, certs are met.
   No artificial difficulty penalties. */
export function areaCertRd(area) {
  ...
  /* Each cert requirement weighted by criticality.
     Coverage = staff fill rate (present staff = certs met).
     Criticality weight: Essential=2, Important=1, Nice to have=0.5 */
  var wTotal = 0, wMet = 0;
  certs.forEach(function(ct) {
    var cr = typeof ct === "string" ? "Important" : (ct.cr || "Important");
    var w = cw(cr);
    wTotal += w;
    wMet += fillRate * w;
  });
...
/* Top priorities: Essential/Critical gaps across all initiatives */
export function portfolioPriorities(inis) {
  var count = 0;
  inis.forEach(function(ini) {
    (ini.sg || []).forEach(function(g) {
      if (g.i === "Essential" || g.i === "Critical") count += 1;
    });
