...
import { mkIni, mkDepts, mkCircles, mkJobProfiles, LAYOUT_TEMPLATES, CAPABILITY_SETS, hydrateIni } from "@/lib/data";
import useIsMobile from "@/lib/useIsMobile";
import OverviewPage from "@/components/OverviewPage";
import DetailPage from "@/components/DetailPage";
import WizardPage from "@/components/WizardPage";
import ReportPage from "@/components/ReportPage";
import CapabilitySetsPage from "@/components/CapabilitySetsPage";
      ...
      <div id="app-root" style={{ minHeight:"100vh", background:T.bg, color:T.tx, fontFamily:"system-ui,sans-serif", width:cap.mobile?390:"auto", maxWidth:cap.mobile?390:"none" }}>
        <div style={{ height:48, display:"flex", alignItems:"center", justifyContent:"space-between", padding:mob?"0 12px":"0 32px", borderBottom:"1px solid "+T.bd, background:T.sf }}>
          <div style={{ display:"flex", alignItems:"center", gap:mob?6:10 }}>
            ...
            {!mob&&<span style={{ fontSize:11, color:T.td, marginLeft:6, cursor:"pointer" }} onClick={function(){sView("overview");}}>/ Initiatives</span>}
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:mob?6:12 }}>
            {!mob&&<button onClick={function(){sView("capsets");}} style={{ padding:"5px 12px", borderRadius:16, border:"1px solid "+(view==="capsets"?T.ac:T.bd), background:view==="capsets"?T.ac+"14":T.sa, cursor:"pointer", color:view==="capsets"?T.ac:T.tm, fontSize:11, fontFamily:"inherit" }}>Capability Sets</button>}
        ...
        {view === "overview" && <OverviewPage ini={hydratedInis} onOpen={function(i) { sSel(i); sView("detail"); }} onCreate={function() { sView("wizard"); }} onReport={function() { sView("report"); }} onToast={showToast} initSubview={cap.subview} initFilter={cap.filter} />}
        ...
        {view === "capsets" && <CapabilitySetsPage capSets={capSets} setCS={setCS} initiatives={hydratedInis} onBack={function() { sView("overview"); }} onToast={showToast} />}
