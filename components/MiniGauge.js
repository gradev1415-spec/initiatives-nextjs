...
export default function MiniGauge({ v, sz, sw, clr }) {
  ...
  return (
    <svg width={size} height={size} style={{ transform:"rotate(-90deg)", flexShrink:0 }}>
      ...
      <circle cx={size/2} cy={size/2} r={r2} fill="none" stroke={c} strokeWidth={stroke} strokeDasharray={ci} strokeDashoffset={off} strokeLinecap="round" style={isOver ? { filter: "drop-shadow(0 0 3px " + c + "60)" } : {}} />
    </svg>
  );
}
